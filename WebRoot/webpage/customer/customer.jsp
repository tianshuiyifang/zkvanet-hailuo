...
// 	var sessionid_cc=0;
//检查用户是否打开多个会话
	/* var sessionid="0B0D7D55F775B2E473E17B717CD4D442";
...
<title>账户中心- Tracker</title>
<link rel="stylesheet" href="webpage/resource/css/grid.css">
</head>

<body class="body bc-gray min-sidebar">
	<div class="wrapper">
		<!-- header start -->
		...
		<div class="navbar-collapse collapse">
			...
			<div class="navbar-right p-15 p-b0 ta-r">
				<div class="user-time">
						...
						<a  href="meunController.do?businessLog">业务日志</a>&emsp;
...
<nav class="menu-seach bc-fff">
	<div class="container-fluid">
		<ul class="nav navbar-nav fs-14">
				<li id="accountCenter"><a href="meunController.do?main"><i class="fa fa-newspaper-o" aria-hidden="true"></i>&nbsp;账户中心</a></li>
				<li id="customerManagement"><a href="meunController.do?search"><i class="fa fa-group"></i>&nbsp;客户管理</a></li>
				<li id="index"><a target="_blank" href="meunController.do?index"><i class="fa fa-map-o" aria-hidden="true"></i>&nbsp;控制台</a></li>
		</ul>
					...
					success:function(ret){
						if(ret.statusCode==0){
							...
							layer.confirm('密码修改成功', {
								...
								//$("#logoutFrom").submit();
								$.ajax({
									url : _ctx+"rest/loginController/logout",
									async : true,
									cache : false,
									type : "POST",
...
function delCookie(name){
	...
	$("#saveUserInformation").click(function(){
		...
		$.ajax({
			...
			cache : false,
			type : "POST",
			data : {"id" : header_userId,"mobile":$("#edit-modal-phone").val(),"email":$("#edit-modal-email").val()},//$("#edit-modal-userForm").serialize(),
			datatype : "json",
			success : function(ret){
				if(ret.statusCode == 0){
	...
	function stock_ajaxDataFilter(treeId, parentNode, responseData) {
		$.each(responseData.data, function(i, v) {
			...
			//拼接成 (库存/进货)
			v.countStr = "(" + v.deviceCount.stock + "/" + v.deviceCount.repertory + ")";
			...
			<div class="container-fluid">
				<div class="row">
					<div class="customer-leftsidebar p-t15">
						<div class="auto-high bc-fff side-left">
							...
							<div class="side-busines account-side-busines p-tb10">
								<ul>
									<li class="active"><a onclick="toAccountTab()">账户详情</a></li>
									<!-- <li><a onclick="getMessageList(1)">消息中心(<span id="unReadTotal">0</span>未读)</a></li>
									<li ><a onclick="queryFictitiousAccountList();">账号管理</a></li>
									<li><a onclick="showOperation()">操作指引</a></li> -->
								</ul>
							</div>
						</div>
					</div>
					<div class="customer-rightsidebar p-15">
						<div class="tab-con-account b1-ccc bc-fff mh-500">
						
							<div class="main-tab">
								<div class="tab-navbar">
				                      <label  class="icheck-min fr p-t5"><input id="indexView" type="checkbox" >&nbsp;记住默认选项</label>
				                      <ul class="tab-nav tab-auto-nav clearfix">
				                          <li id="zSales" class="active"><a class="cp" onclick="showTotalView(0)">账户总览</a></li>
				                        <!--   <li id="eSales"><a class="cp" onclick="showTotalView(1)">快速销售</a></li>
				                    -->   </ul>
				                  </div>
								<div class="tab-con">
									<div class="overview-con-box p-25">
										<div class="row metor-grid">
			                              <div class="col-xs-6 p-r15">
			                                  <div class="row m-r0 m-b5 m-lr0">
			                                      <div class="col-xs-6">
			                                          <a class="grid bc-7556D6" href="meunController.do?search" target="_blank">
			                                              <i class="icon icon-home"></i>
			                                              <span class="text"><b>库存</b>(<span class="num" id="stock2">${stockCount}</span>)</span>
			                                          </a>
			                                      </div>
			                                      <div  class="col-xs-6">
			                                          <a class="grid bc-7F3979"  href="meunController.do?search" target="_blank">
			                                              <i class="icon icon-shop-cart"></i>
			                                              <span class="text"><b>总进货数</b>(<span class="num" id="repertory2">${DeviceCount}</span>)</span>
			                                          </a>
			                                      </div>
			                                      <div class="col-xs-3">
			                                          <a class="grid bc-3FA0EC"  href="meunController.do?index" target="_blank">
			                                              <i class="icon icon-cart"></i>
			                                              <span class="text"><b>在线</b>(<span class="num" id="onLine2">${OnlineCount}</span>)</span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-3">
			                                          <a class="grid bc-1B9CB2"  href="meunController.do?index" target="_blank">
			                                              <i class="icon icon-cart-gray"></i>
			                                              <span class="text"><b>离线</b>(<span class="num" id="noOnLine2">${OfflineCount}</span>)</span>
			                                          </a>
			                                      </div>
			                                      <div class="col-xs-3">
			
			                                          <a class="grid bc-31aa2d"  href="meunController.do?search" target="_blank">
			                                              <i class="icon icon-hourglass"></i>
			                                              <span class="text"><b>即将到期</b>(<span class="num" id="devExpiring2">${NearExpireCount}</span>)</span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-3">
			
			                                          <a class="grid bc-2F5998"  href="meunController.do?search" target="_blank">
			                                              <i class="icon icon-history"></i>
			                                              <span class="text"><b>已过期</b>(<span class="num" id="devExpired2">${ExpireCount}</span>)</span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-3">
			
			                                          <a class="grid bc-3b9f3d"  href="meunController.do?search" target="_blank">
			                                              <i class="icon icon-magic"></i>
			                                              <span class="text"><b>已激活</b>(<span class="num" id="active2">${ActivedCount}</span>)</span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-3">
			                                          <a class="grid bc-2f61ad"  href="meunController.do?search" target="_blank">
			                                              <i class="icon icon-target"></i>
			                                              <span class="text"><b>未激活</b>(<span class="num" id="noActive2">${UnActivedCount}</span>)</span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-3">
			                                          <a class="grid bc-e96343 cp" onclick="alarmSwitch(this);">
			                                              <i class="icon icon-warning"></i>
			                                              <span class="text"><b>告警车辆</b>(<span class="num" id="carAlarm">0</span>)</span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-3">
			                                          <a class="grid bc-2f9bf2"  href="/index?userId=16341&viewFlag=4" target="_blank">
			                                              <i class="icon icon-heart"></i>
			                                              <span class="text"><b>重点关注车辆</b>(<span class="num" id="followNum">0</span>)</span>
			                                          </a>
			                              ...
			                              <div class="col-xs-6 p-l15">
			                                  <div class="row m-l0 m-lr0">
			                                      <div class="col-xs-4">
			                                          <a class="grid bc-01AFF0"  href="meunController.do?index" target="_blank">
			                                              <i class="icon icon-monitor"></i>
			                                              <span class="text"><b>控制台</b></span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-8">
			                                          <a class="grid bc-3E5C8A"  href="meunController.do?report" target="_blank">
			                                              <i class="icon icon-signal"></i>
			                                              <span class="text"><b>统计报表</b></span>
			                                          </a>
			                                      </div>
			                                      <div class="col-xs-4">
			                                          <a class="grid bc-0086D0"  href="meunController.do?setUp" target="_blank">
			                                              <i class="icon icon-enclosure"></i>
			                                              <span class="text"><b>围栏</b></span>
			                                          </a>
			                                      </div>
			                                      <div class="col-xs-4">
			                                          <a class="grid bc-7F3979"  href="meunController.do?search" target="_blank">
			                                              <i class="icon icon-face-set"></i>
			                                              <span class="text"><b>下级客户管理</b></span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-4">
			                                          <a class="grid bc-008299"  href="meunController.do?device" target="_blank">
			                                              <i class="icon icon-router"></i>
			                                              <span class="text"><b>设备管理</b></span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-4">
			                                          <a class="grid bc-2770ec"  href="meunController.do?template" target="_blank">
			                                              <i class="icon icon-note"></i>
			                                              <span class="text"><b>指令管理</b></span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-4">
			                                          <a class="grid bc-0F7B11"  href="meunController.do?setUp" target="_blank">
			                                              <i class="icon icon-flag"></i>
			                                              <span class="text"><b>地标设置</b></span>
			                                          </a>
			
			                                      </div>
			                                      <div class="col-xs-4">
			                                          <a class="grid bc-A34A42"  href="#" target="_blank">
			                                              <i class="icon icon-bell-set"></i>
			                                              <span class="text"><b>告警邮件提醒设置</b></span>
			                                          </a>
			                          	  ...
			                          	  <div class="col-xs-12">
			                          	  	<div class="row c-fff clearfix">
			                          	  		<div class="col-xs-3">
			                          	  			<div class="account-information bc-057DC3 m-l15">
			                          	  				<div class="media p-15 p-b10 m-0">
															<div class="media-left">
																<span
																	...
																	><i
																	class="fa fa-user fa-4x"></i></span>
															</div>
															<div class="media-body">
																<h4 class="media-heading" id="userName"></h4>
																<ul class="lh-2">
																	<li><b>账号</b>：<span
																		id="userAccount">${userName}</span></li>
																	<li><b>客户类型</b>：<span
																		id="userType">${userType}</span></li>
																	<li><b>电话</b>：<span
																		id="userPhone">${userPhone}</span></li>
																</ul>
																
																<input type="hidden" id="userId" value="16341" />
															</div>
														</div>
			                          	  			</div>
			                          	  		</div>
			                          	  		<div class="col-xs-6 p-l5">
			                          	  			<div class="service-provider bc-057DC3 m-r5">
				                          	  			<div class="provider-scroll oy-a clearfix">
			                          	  		...
			                          	  		<div class="col-xs-3 p-l0">
			                          	  			<div class="app-qrcode ta-c p-tb5 bc-057DC3 m-r15">
			                                              <img class="d-ib m-t5" src="http://statics.jimicloud.com/tracker/domain/lbs.tuyouonline.com_qrcode.png" width="80" height="80"> <br>
			                                              <span class="p-t9  d-ib">扫描二维码，下载手机APP客户端</span>
			                                          </div>
			                          	  		</div>
			                          	  	</div>
			                          	  </div>
									...
									<div class="account-con-box row p-25">
										<div class="col-md-8 p-r0">
											<div class="bc-fff equipment-sales b1-ccc">
												<div class="section-header">
													<b>快速销售</b>
												</div>
												<div class="p-15 ">
													<div class="form-horizontal form-ellipsis">
														<div class="form-group m-b0">
															<label class="col-sm-2 control-label"
																title="销售给"> <span
																class="fl"><span class="c-red">*</span>
																销售给：</span></label>
															<div class="col-md-7 p-l0 select-user">
																<input type="hidden" id="selectedUserId" value="">
																<input type="hidden" id="selectedParentId" value="">
																<div class="input-group input-group-sm m-b10 z-i3">
																	<input type="text" id="selectUserSalesName"
																		class="form-control js-select-user-sales"
																		readonly="readonly"
																		title="请选择用户"
																		placeholder="请选择用户"
																		size="25"> <span class="input-group-btn">
																		<button title="" id="showTree-btn"
																			class="btn btn-default js-select-sales-btn"
																			type="button">
																			<i class="fa fa-caret-down"></i>
																		</button>
																	</span>
																</div>
																<div class="pr user-sales-box">
																	<div class="js-select-user-sales-tree-box select-user-sales-tree-box b1-ccc p-lr10 pa bc-fff dn">
																		<div class="tree-search m-t10">
																				<i class="tree-search-icon"
																					id="ac_putDevice_globalSearch_btn"></i> <input
																					type="text" id="ac_putDevice_globalSearch_SalesName"
																					class="empty fs-12" value=""
																					placeholder="请输入客户名称或帐号">
																			</div>
																		<div class="oy-a  h300">																		
																			<div class="ta-c m-tb15" id="TreeLogindata">
																				<i class="fa fa-spinner fa-pulse fa-fw"></i>&nbsp;
																				正在加载数据...
																			</div>
																			<ul id="ac_putDevice_custTree" class="ztree">
																			</ul>
																		</div>
																	</div>
																</div>
															...
															<div class="col-sm-3 p-l0">
																<button class="btn btn-default btn-sm js-add-customers"
																	type="button">
																	新增客户
																</button>
															</div>
														</div>
														<div class="form-group m-b0 p-b10">
															<label class="col-sm-2 control-label"
																title="追加设备">
																<span class="fl"><span class="c-red">*</span>
																追加设备：</span>
															</label>
															<div class="col-sm-7 p-l0">
																<div
																	class="form-control form-control-sm imei-textarea-box pr">
																	<textarea type="text" id="searchIMEI"
																		class="imei-textarea"
																	...
																	<div class="func-btnbar clearfix">
																		...
																		<div id="putdevice_loading" class="fl p-t5 p-l15 dn">
															...
															<label class="col-sm-3 control-label p-l0">
																<p class="ib fl">
																	<span
																		title="已选设备">已选设备 ：</span><span class="c-red"
													...
													<div class="p-t15 select-validity">
														<button class="btn btn-sm btn-default fr m-lr5 mw-100"
															id="resetDevice">
															重置
														</button>
														...
														<button class="btn btn-sm btn-primary fr mw-100 dev-tohandle"
															onclick="saleDevice();">
															销售
														</button>
														<lable>
														用户到期时间：</lable>
														<span
															class="easydropdown easydropdown-up easydropdown-sm d-ib va-m">
															<select class="js-select" id="expiredType">
										...
										<div class="col-md-4">
			                              <div class="subuser-tree bc-fff clearfix b1-ccc">
			                                  <div class="section-header"><b>下级客户&nbsp;库存</b></div>
			                                  <div class="p-lr10">
			                                  	  <div class="tree-search m-t10"><i class="tree-search-icon" id="treeSubUser_search_btn"></i>
			                                              <input type="text" class="empty fs-12" id="treeSubUser_search_input" value="" placeholder="搜索">
			                                              <!--<input type="radio"  name="keyType" class="hide" checked/>
			                                              <input type="radio" checked="" style="margin-left:36px;" class="hide" name="funType" >-->
			                                          </div>
			                                      <div class="auto-scrollbar">
			                                          
			                                          <ul class="ztree" id="treeSubUser"  >
			                                          </ul>
			                                      </div>
			                                  </div>
			                              </div>
						...
						<div class="tab-con-notice  b1-ccc bc-fff mh-500 dn">

							<div class="rightside-header">
								<div class="p-tb5">
									<b>消息中心</b>&nbsp;<i
										id="alarmReceivingTip"
										class="fa fa-question-circle c-F0AD4E"
										data-content="如需邮箱也接收相应的推送提醒，请在个人资料里完善自己的邮箱信息，并确保邮箱可正常接收邮件"></i>
								</div>
							</div>
							<div class="right-tab-con">
											...
											<div class="pr d-ib">
												<div class="input-group input-group-sm">
													<input type="text"
														...
														id="remainSearchDeviceInput">
												</div>
											</div>
											&nbsp; <span class="easydropdown easydropdown-sm va-m">
												<select class="js-select messageType">
													<option value="">消息类型</option>
													<option value="2">设备离线</option>
													<option value="1">设备到期</option>
											</select>
											</span> &nbsp; <span class="easydropdown easydropdown-sm va-m">
												<select class="js-select msgStatus">
													<option value="">状态</option>
													<option value="0">未读</option>
													<option value="1">已读</option>
											</select>
											</span> &nbsp;
											<button type="button" onclick="getMessageList(1)"
								...
								<div class="table-header">
									<table id="noticeTableHeader"
										class="table table-hover table-ellipsis m-b0">
										<thead>
											<tr>
												<th><label class="icheck-min fw-b"><input type="checkbox" class="msg-all-check">&nbsp;序号</label></th>
												<th>状态</th>
												<th>发送时间</th>
												<th>内容</th>
												<th>消息类型</th>
								...
								<div class="table-scrollbar oy-a">
									<table id="noticeTableContent"
										class="table table-hover table-ellipsis">
										<colgroup>
											<col width="100">
											<col width="8%">
											<col width="15%">
									...
									<div id="msg_nodata" style="display: none;">
										<div class="ta-c c-666 p-tb10">
											<span class="p-5 d-ib">&nbsp;<i class="fa fa-bell-o"></i>
											暂无数据&nbsp;
											</span>
										</div>
									</div>
									<div id="userMessage_logindata" class="ta-c m-tb15" style="display: none;">
										<i class="fa fa-spinner fa-pulse fa-fw fa-2x"></i>
										正在加载数据...
									</div>
									<div id="msg_paging" class="simple-pagination-custom ta-c"></div>
								</div>
							</div>
						</div>
						<div class="tab-con-virtual b1-ccc bc-fff dn">
                            <div class="rightside-header">
                                <div class="p-tb5">
                                    <b>账号管理</b>
                                </div>
                            ...
                            <div class="right-tab-con">
                                <div class="funcbar">
                                    <form class="form-inline clearfix p-tb10">
                                        <div class="form-group fl">
                                            <button type="button" class="btn btn-default btn-sm js-add-virtual-btn">添加</button>
                                        </div>
                                    </form>
                                </div>
                                <div class="table-header">
                                    <table id="virtualTableHeader" class="table table-hover table-ellipsis m-b0">
                                        <thead>
                                            <tr>
                                                <th title="序号">序号</th>
                                                <th title="账号登陆名称">账号登陆名称</th>
                                ...
                                <div class="table-scrollbar oy-a">
                                    <table id="virtualTableContent" class="table table-hover">
                                        <colgroup>
                                            <col width="100">
                                            <col width="200">
                                            <col width="50%">
                                        ...
                                        <tbody id="fictitiousAccount_tbody">
                                        </tbody>
                                    </table>
                                    <div class="ta-c m-tb15" id="fictitiousAccount_loadingdata">
										<i class="fa fa-spinner fa-pulse fa-fw"></i>&nbsp;
										正在加载数据...
									</div>
                                    <div id="fictitiousAccount_nodata" style="display: none;">
										<div class="ta-c c-666 p-tb10">
											<span class="p-5 d-ib">&nbsp;<i class="fa fa-bell-o"></i>
											暂无数据&nbsp;
											</span>
						...
						<div class="tab-con-operation  b1-ccc bc-fff mh-500 dn">
							...
							<div class="right-tab-con">
								<iframe width="100%" class="db" id="operationIframe" frameborder="0"></iframe>
							</div>
						</div>
...
<script type="text/javascript">var cnzz_protocol = (("https:" == document.location.protocol) ? " https://" : " http://");document.write(unescape("%3Cspan id='cnzz_stat_icon_1260225997'%3E%3C/span%3E%3Cscript src='" + cnzz_protocol + "s11.cnzz.com/z_stat.php%3Fid%3D1260225997%26show%3Dpic' type='text/javascript'%3E%3C/script%3E"));</script>
			...
			<div class="modal-content w600">
				...
				<div class="modal-body">
					<div class="">
						<div>
							<p>
								当前添加成功&thinsp;<b class="c-72af2d"><span id="succCount"></span></b>&nbsp;&nbsp;
								, 失败 &thinsp;<b class="c-red"><span id="failCount"></span></b>
							</p>
						</div>
	...
	<div class="modal fade" id="addRole">
		<div class="modal-dialog">
			<div class="modal-content w800">
				...
				<div class="modal-body">
					<div class="p-tb15 mah-450 oy-a">
						<form id="addRole_userForm"
							class="form-horizontal form-preset form-sm form-ellipsis mh-400"
							action="">
							<input type="hidden" id="parentId" name="parentId" /> <input
								type="hidden" name="fullPid"> <input type="hidden"
								id="userId">
							<div class="form-group">
								...
								<div class="col-md-4">
									<fieldset id="selectRole">
										<div class="input-group">
											<input id="topUser" name="parentName" type="text"
												class="form-control form-control-sm " value="" readonly>
											<span class="input-group-btn">
												<button type="button"
													class="btn btn-default btn-sm direction-btn">
								...
								<div class="col-md-5 pr">
									<div class="pa p-10 tree-role-box" id="treeRoleBox"
										...
										<div class="tree-search">
											<i class="tree-search-icon" id="ac_addRole_globalSearch_btn"></i>
											<input type="text" id="ac_addRole_globalSearch_SalesName"
												class="empty fs-12" value=""
												placeholder="请输入客户名称或帐号">
										</div>
										<div class="tree-scroll-role auto-scrollbar h350">
											<div class="ta-c m-tb15" id="ac_addRole_TreeLogindata">
												<i class="fa fa-spinner fa-pulse fa-fw"></i>&nbsp;
												正在加载数据...
											</div>
											<ul id="ac_addRole_custTree" class="ztree">
											</ul>
										</div>
									</div>
								</div>
							...
							<div class="form-group">
								...
								<div class="col-md-4">
									<div class="p-tb8">
											...
											<label class="icheck-min"> <input type="radio"
												name="type" value="8" checked>&nbsp; 代理商
											</label>
											&emsp;
										<label class="icheck-min"> <input type="radio"
											name="type" value="9" checked>&nbsp;用户
										</label>
									</div>
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-md-4">
									<input id="nickName" name="nickName" maxlength="50" type="text"
							...
							<div class="form-group">
								...
								<div class="col-md-4">
									<input id="account" name="account" maxlength="30" type="text"
										class="form-control form-control-sm"
										placeholder="请输入账号" />
								</div>
							</div>
							<div id="markPassword">
								...
								<div id="markPswAgain" class="form-group" style="display: none;">
									<label class="col-md-3 control-label"
										title="确认密码">
									...
									<div class="col-md-4">
										<input id="pswAgain" name="pswAgain" type="password"
											class="form-control form-control-sm"
							...
							<div class="form-group">
								...
								<div class="col-md-4">
									<input id="phone" name="phone" maxlength="20" type="text"
										class="form-control form-control-sm"
										placeholder="请输入电话号码" />
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-md-2">
									<label class="icheck-min text-ellipsis p-tb5"
										title="Web登录权限"> <input
										type="checkbox" name="webLogin" checked="checked" />&nbsp;Web登录权限
									</label>
								</div>
								<div class="col-md-2">
									<label class="icheck-min text-ellipsis p-tb5"
										title="APP登录权限"> <input
										type="checkbox" name="appLogin" checked="checked" />&nbsp;APP登录权限
									</label>
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-md-2">
									<label class="icheck-min text-ellipsis p-tb5"
										title="批量下发指令"> <input
										type="checkbox" name="isBatchSendIns" />&nbsp;批量下发指令
									</label>
								</div>
								<div class="col-md-2">
									<label class="icheck-min text-ellipsis p-tb5"
										title="批量下发工作模式"> <input
										type="checkbox" name="isBatchSendFM" />&nbsp;批量下发工作模式
									</label>
								</div>
							</div>
				...
				<div class="modal-footer">
					<button id="addUserBtn" type="button"
						class="btn btn-primary mw-100" onclick="saveUser();">
						保存
					</button>
					<button type="button" class="btn btn-default mw-100"
						data-dismiss="modal">
	...
	<!-- 延长到期 -->
	<div class="modal fade" id="allotDevProlong_accountCenter">
		<div class="modal-dialog">
			<div class="modal-content w800">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"
				...
				<div class="modal-body">
					<input type="hidden" id="prolong-check" />
					<div class="p-tb25 row">
						<div class="col-md-8">
							<label> 已选设备列表：
							</label>
							<div class="b1-ccc js-prolong-scroll h350 oy-a">
								<table class="table table-condensed table-ellipsis">
									<thead>
										<th width="180"
											title="设备名  ">设备名  </th>
										<th width="140" title="IMEI">IMEI</th>
										<th title="用户">用户</th>
									</thead>
									<tbody id="allotProlong_accountCenter">
									</tbody>
								</table>
							</div>
						</div>
						<div class="col-md-4">
							...
							<div class="b1-ccc js-prolong-package-scroll h350 oy-a">
								<ul id="prolongPackage_accountCenter"
									class="package-list list-fl p-15 js-package-list"></ul>
							</div>
						</div>
					</div>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-primary mw-100"
						onclick="complex_allotDevProlong('accountCenter');">
						保存
					</button>
					&nbsp;
					<button type="button" class="btn btn-default mw-100"
	...
	<!-- 新增账号 -->
    <div class="modal fade" id="addVirtualAccountModal">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">添加/编辑账号</h4>
                </div>
                <div class="modal-body">
                    <form id="fictitiousAccountForm" class="form-horizontal form-preset  form-ellipsis p-tb25" action="">
                        <input type="hidden" name="fictitious_id" />
                        <div class="form-group">
                            <label class="col-md-3 control-label" title="账号登陆名称">账号登陆名称</label>
                            <div class="col-md-6">
                                <input type="text" name="fictitious_account" placeholder="请输入账号名称" class="form-control">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-3 control-label" title="密码">密码：</label>
                            <div class="col-md-6">
                                <input type="password" name="fictitious_password" id="fictitious_password" placeholder="请输入密码" class="form-control">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-3 control-label" title="确认密码">确认密码：</label>
                            <div class="col-md-6">
                                <input type="password" name="fictitious_passwordAgain" placeholder="请确认您输入的密码" class="form-control" id="password">
                            </div>
                        </div>
                      <!--   <div class="form-group">
                            <label class="col-md-3 control-label" title="权限范围">权限范围：</label>
                            <div class="col-md-6">
                              <ul class="lh-2">
                                <li class="p-b5"><label class="icheck-min"><input type="checkbox" value="0" name="fictitious_updateFlag"/>&nbsp;修改数据</label></li>
                                <li><label class="icheck-min"><input type="checkbox" value="0" name="fictitious_instructionsFlag"/>&nbsp;下发指令</label></li>
                              </ul>
                            </div>
                        </div> -->
                    </form>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary mw-100 addFictitious" onclick="addFictitiousAccount();">保存</button>
                    <button type="button" class="btn btn-primary mw-100 updateFictitious" onclick="updateFictitiousAccount();">保存</button>
		...
		$("#alarmSound").iCheck().on("ifClicked",function(){
			if(!$("#alarmSound").attr("checked")){
				...
				if($("input[name='markAlarmId']").length>0){
					audioPlay("/resource/media/alarm.wav");//播放
				}
			}else{
			    audioPlay("");//暂停
	...
	function bindAlarmInfo(bdlat,bdlng,gglat,gglng,alarmInfoIndex){
		if($("#index").hasClass("active")){
			if($("#selectMap").val()=="baiduMap"){
				var stopFlagIcon=_ctx+"/resource/images/36x43/alarm.png";
				...
				var stopFlagIcon = {url:_ctx+"/resource/images/36x43/alarm.png" ,size:new google.maps.Size(23, 27),origin:new google.maps.Point(0,0),anchor:new google.maps.Point(3,26)};
...
<!--[if lt IE 9]><link rel="stylesheet" href="/resource/plugins/photobox/photobox.ie.css"><![endif]-->
<script src="webpage/resource/plugins/photobox/jquery.photobox.js"></script> 

</body>
</html>
<script type="text/javascript" src="webpage/resource/js/userMessage.js"></script>
<script type="text/javascript" src="webpage/resource/js/customer/FictitiousAccount.js"></script>
<script type="text/html" id="msg_tbody_tmp">
{{each data as row i}}
<tr {{if row.readFlag != 0}}class="c-999"{{/if}}>
	<td><label class="icheck-min"><input type="checkbox" value="{{row.id}}" class="msg-check" {{if row.readFlag != 0}}disabled="disabled"{{/if}}>&nbsp;{{(pageNo-1)*pageSize+(i+1)}}</label></td>
	<td>{{if row.readFlag == 0}}<span class="c-72AF2D">未读</span>{{else}}已读{{/if}}</td>
	<td>{{row.pushTime}}</td>
    <td><div class="td-normal">{{row.number,row.type | getContent}}{{#row.imeis | getButton}}</div></td>
	<td>
		{{if row.type == 1}}设备到期{{/if}}
		{{if row.type == 2}}设备离线{{/if}}
	</td>
	<td>系统</td>
</tr>
{{/each}}
</script>
<script type="text/html" id="fictitiousAccount_tbody_tmp">
	{{each data as row i}}
		<tr>
	      <td>{{i+1}}</td>
	      <td>{{row.account}}</td>
	      <td>{{row.creationDate}}</td>
	      <td><a class="js-edit-virtual-btn" onclick="openUpdateFictitiousAccountModal('{{row.id}}')">编辑</a>&emsp;<a onclick="deleteFictitiousAccount('{{row.id}}',this)">删除</a></td>
	    </tr>
    {{/each}}
</script>
...
<script type="text/javascript" src="webpage/resource/js/accountCenter.js"></script>
<script type="text/javascript">
	var userId = "16341";
	var account = "yanshi";
</script>
<script type="text/javascript" src="webpage/resource/js/user/userHeightCount.js"></script>
<script>


//操作指引跳转
function showOperation(){
	$("#operationIframe").attr("src","/OperationGuide");
}


//
$("#alarmReceivingTip").popover({
	placement:"right",
	trigger:"hover"
});

	//iCheck
	$("input[type='radio'][name='type'],input[type='checkbox'][name='webLogin'],input[type='checkbox'][name='appLogin'],[name='permis_editdata'],[name='fictitious_instructionsFlag'],[name='fictitious_updateFlag']").iCheck();
	$("#addRole_userForm input[name=isBatchSendIns]").iCheck();
	$("#addRole_userForm input[name=isBatchSendFM]").iCheck();
	
	//勾选默认模块控制
	$("#indexView").iCheck();
	var defaultView = getCookie("defaultselected");
	if(defaultView){
		$("#indexView").iCheck('check');
		if(defaultView == "1"){
			$(".main-tab .tab-nav>li").eq(defaultView).trigger("click");
		}
	}
	
	$("#indexView").on("ifClicked",function(){
		var index = $(".main-tab .tab-nav>li[class*='active']").index();
		if(!$("#indexView").attr("checked")){
			SetCookie("defaultselected",index);
		}else{
			delCookie("defaultselected");
		}
	});
	
	function showTotalView(index){
		if(index == 0 && !$(".overview-con-box").is(":visible")){
			//getUserAndDevNum();
		}
		var defaultView = getCookie("defaultselected");
		if(!defaultView || defaultView != index){
			$("#indexView").iCheck('uncheck');
		}else{
			$("#indexView").iCheck('check');
		}
	}
	...
	//选择用户销售
	$("#showTree-btn").click(function() {
		var flag = $(this).find(".fa").hasClass("fa-caret-up");
		salesUserTreeToggle(flag);
	});
	
	//选择用户销售
	$("#selectUserSalesName").click(function() {
		var flag = $(".js-select-user-sales-tree-box").is(":visible");
		salesUserTreeToggle(flag);
	});
	
	//销售用户树切换显示
	function salesUserTreeToggle(flag) {
		if (!flag) {
			$(".js-select-user-sales-tree-box").show();
			$(".js-select-sales-btn").find(".fa").attr("class","fa fa-caret-up");
			$("#ac_putDevice_globalSearch_SalesName").focus();
		} else { 
			$(".js-select-user-sales-tree-box").hide();
			$(".js-select-sales-btn").find(".fa").attr("class","fa fa-caret-down");
		}
		selectUserSalesTreeClickOutHide();
	}

	//鼠标如果没有点击在选择树的元素上，元素隐藏
	function selectUserSalesTreeClickOutHide() {
		$(document).bind("click",function(e) {
			var target = $(e.target);
			if (!target.closest(".select-user").length) {
				$(".js-select-user-sales-tree-box").hide();//hide(); 可以设置隐藏或销毁
				$(document).unbind("click");
				$(".js-select-sales-btn>.fa").attr("class","fa fa-caret-down");
			}
		});
	}
	
	$("[name=roleRadio1]").iCheck().on('ifChecked', function(event) {
		if ($(this).val() == 1) {
			$("#editorSelectRole").attr("disabled", true);
			$("#editorTreeRoleBox").hide();
		} else {
			$("#editorSelectRole").attr("disabled", false);
			$("#editorTreeRoleBox").hide();
			if ($(this).val() == 2) {
	...
	$("input[name='fictitious_updateFlag']").iCheck().on('ifChecked',function(event){
		$("#addVirtualAccountModal").find("input[name='fictitious_updateFlag']").val("1");
	}).on('ifUnchecked',function(event){
		$("#addVirtualAccountModal").find("input[name='fictitious_updateFlag']").val("0");
	});
	
	$("input[name='fictitious_instructionsFlag']").iCheck().on('ifChecked',function(event){
		$("#addVirtualAccountModal").find("input[name='fictitious_instructionsFlag']").val("1");
	}).on('ifUnchecked',function(event){
		$("#addVirtualAccountModal").find("input[name='fictitious_instructionsFlag']").val("0");
	});
	
	//新增账号
    $(".js-add-virtual-btn,.js-edit-virtual-btn").click(function(){
      $("#addVirtualAccountModal").find("input[name^='fictitious_']").val("");
      $("#addVirtualAccountModal").find("input[name='fictitious_account']").removeAttr("readonly");
   	  $("#addVirtualAccountModal").find("input[name='fictitious_updateFlag']").val("0").iCheck('uncheck');
   	  $("#addVirtualAccountModal").find("input[name='fictitious_instructionsFlag']").val("0").iCheck('uncheck');
   	  
   	  clearFormMsg()   	  
   	  $(".addFictitious").show();
	  $(".updateFictitious").hide();
