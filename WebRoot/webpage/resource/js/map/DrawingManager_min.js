 ...
 (function() {
   ...
   DrawingManager.prototype._bindCircle = function() {
       ...
       var startAction = function (e) {
           ...
           mask.addEventListener('dblclick', endAction);
           //双击时候不放大地图级别
       ...
       var endAction = function (e) {
           ...
           mask.removeEventListener('dblclick', endAction);
       }
   ...
   function setBubbleFocus(){
	   if(consoleGeo){
		   $("input[name = 'mapAlarmType'],input[name = 'mapPush']").iCheck();
		   document.getElementById("bdradius").value = radius;
	   }
	  
	   setTimeout(function(){
	       document.getElementById("geonameHtml").focus();
	       if(geoorlamk == 2){
	    	   var carRadius = $("#radius").val();
			   document.getElementById("bdchangeRadius").value = parseInt(carRadius);
		   }
	   },100);
   }
    ...
    Mask.prototype.initialize = function(map){
        ...
        var setCursor = "";
        if(polyType == "hander"){
        	setCursor = "url('bird.cur')";
        }else{
        	setCursor = "crosshair";
        }
        div.style.cssText = "position:absolute;background:url(about:blank);cursor:"+setCursor+";width:" + size.width + "px;height:" + size.height + "px";
    ...
    DrawingTool.prototype._bindEventByDraingMode = function(drawingType){
        ...
        if (drawingType == "hander") {
            ...
            drawingManager.setDefaultCursor("url('bird.cur')");
        } else {
