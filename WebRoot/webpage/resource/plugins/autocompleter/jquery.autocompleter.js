                    ...
                    if (response.length < data.limit) {
                        ...
                        case 0:
                        	if(label && typeof(label) != 'undefined'){
                        		if (label.toUpperCase().search(query.replace('\\','\\\\')) === 0) {
                        ...
                        case 1:
                        	if(label && typeof(label) != 'undefined'){
                        		if (label.toUpperCase().search(query.replace('\\','\\\\')) !== -1) {
                        			response.push(source[item]);
                        			delete source[item];
                        		}
                        	}
    ...
    function _buildList(list, data) {
        ...
        for (var item = 0, count = list.length; item < count; item++) {
            var classes = ['autocompleter-item'],
                highlightReg = new RegExp(data.query.replace('\\','\\\\'), 'gi');
        ...
        if (list.length && data.hint) {
            ...
            if (hint && (data.query !== hintLabel)) {
                var hintReg = new RegExp(data.query.replace('\\','\\\\'), 'i');
