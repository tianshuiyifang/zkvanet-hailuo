...
Dates.check = function(){
    ...
    if(value.replace(/\s/g, '') !== ''){
        if(!exp.test(value)){
            Dates.elem[as.elemv] = '';
            Dates.msg($.i18n.prop('comm.Datedoesnotconformtotheformatpleasereselect'));
            return 1;
        } else if(isvoid[0]){
            Dates.elem[as.elemv] = '';
            Dates.msg($.i18n.prop('comm.Thedateisnotvalidpleasereselect'));
            return 1;
        } else {
...
Dates.viewDate = function(Y, M, D){
    ...
    //锁定年月
    as.year.value = Dates.ymd[0] + $.i18n.prop('comm.year');
    as.month.value = Dates.digit(Dates.ymd[1] + 1) + $.i18n.prop('comm.month');
...
Dates.festival = function(td, md){
    ...
    switch(md){
        case '1.1':
            str = $.i18n.prop('comm.NewyearsDay');
        break;
        case '3.8':
            str = $.i18n.prop('comm.Woman');
        break;
        case '4.5':
            str = $.i18n.prop('comm.ChingMing');
        break;
        case '5.1':
            str = $.i18n.prop('comm.labor');
        break;
        case '6.1':
            str = $.i18n.prop('comm.Childrens');
        break;
        case '9.10':
            str = $.i18n.prop('comm.Teachers');
        break;
        case '10.1':
            str = $.i18n.prop('comm.National');
        break;
    };
...
Dates.viewYears = function(YY){
    ...
    Dates.each(new Array(14), function(i){
        if(i === 7) {
            str += '<li '+ (parseInt(as.year.value) === YY ? 'class="'+ as[2] +'"' : '') +' y="'+ YY +'">'+ YY + $.i18n.prop('comm.year') +'</li>';
        } else {
            str += '<li y="'+ (YY-7+i) +'">'+ (YY-7+i) + $.i18n.prop('comm.year') + '</li>';
        }
    }); 
...
Dates.viewtb = (function(){
    var tr, view = [], weeks = [$.i18n.prop('comm.Sun'), $.i18n.prop('comm.Mon'), $.i18n.prop('comm.Tue'), $.i18n.prop('comm.Wed'), $.i18n.prop('comm.Thu'), $.i18n.prop('comm.Fri'), $.i18n.prop('comm.Sat')];
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
        ...
        var displayHms = '<li><input readonly>:</li>'+'<li><input readonly></li>'
        if(options.elem == "#startTime_stopCar" || options.elem == "#endTime_stopCar" || options.elem == "#startTime_geozone" || options.elem == "#endTime_geozone"){//停留和围栏告警只显示时分
        	displayHms = '<li><input readonly></li>'+'<li style="display: none;"><input readonly></li>'
        }
          ...
          +'<div class="laydate_ym laydate_m" id="laydate_MM">'
            ...
            +'<div class="laydate_yms" id="laydate_ms">'+ function(){
                ...
                Dates.each(new Array(12), function(i){
                    str += '<span m="'+ i +'">'+ Dates.digit(i+1) + $.i18n.prop('comm.month') + '</span>';
                });
                return str;
            }() +'</div>'
          +'</div>'
        ...
        +'<div class="laydate_bottom">'
          +'<ul id="laydate_hms">'
            +'<li class="laydate_sj">'+ $.i18n.prop('header.MileageReport.Time') +'</li>'
            +'<li><input readonly>:</li>'
            + displayHms
          +'</ul>'
          +'<div class="laydate_time" id="laydate_time"></div>'
          +'<div class="laydate_btn">'
            +'<a id="laydate_clear">'+ $.i18n.prop('comm.Empty') +'</a>'
            +'<a id="laydate_today">'+ $.i18n.prop('comm.Today') +'</a>'
            +'<a id="laydate_ok">'+ $.i18n.prop('Asset.yes') +'</a>'
          +'</div>'
...
Dates.events = function(){
    ...
    log.hmss = [$.i18n.prop('comm.commHour'), $.i18n.prop('comm.commMinute'), $.i18n.prop('comm.commSeconds')];
    log.hmsarr = [];
    
    //生成时分秒或警告信息
    Dates.msg = function(i, title){
        var str = '<div class="laydte_hsmtex">'+ (title || $.i18n.prop('comm.tips')) +'<span>×</span></div>';
        if(typeof i === 'string'){
            str += '<p>'+ i +'</p>';
            Dates.shde(S('#'+as[0]));
