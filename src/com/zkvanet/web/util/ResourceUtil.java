...
public class ResourceUtil {
	...
	public static final UserInfoDto getSessionUserName() {
		HttpSession session = ContextHolderUtils.getSession();
		if(ClientManager.getInstance().getClient(session.getId())!=null){
			return ClientManager.getInstance().getClient(session.getId()).getUserInfo();
		//update-begin--update---author:scott-----------date:20151218-------for:解决分布式登录问题-------
		}else{
			UserInfoDto u = (UserInfoDto) session.getAttribute(ResourceUtil.LOCAL_CLINET_USER);
			Client client = new Client();
	        client.setLogindatetime(new Date());
	        client.setUserInfo(u);
